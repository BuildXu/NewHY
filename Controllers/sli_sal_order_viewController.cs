...
{
    public class SliSalOrderController : ApiController
    {
        public SliSalOrderController()
        {
        ...
        public IHttpActionResult GetTable(int page = 1, int pageSize = 10, string fbillNumber = null)
        {
            var context = new YourDbContext();
            ...
            if (!string.IsNullOrEmpty(fbillNumber))
            {
                query = query.Where(q => q.Sli_sal_order.FBILLNO.Contains(fbillNumber));
            }
