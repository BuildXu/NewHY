...
namespace WebApi_SY.Controllers
{
    public class sli_sal_deliverynoticeController : ApiController
    {
        ...
        public IHttpActionResult DateSync(sli_sal_deliverynotice_view delivery)
        {
            try
            {
                var context = new YourDbContext();
                ...
                if (bLogin)
                {
                    ...
                    var heard = context.T_SAL_DELIVERYNOTICE.FirstOrDefault(p => p.Fid == delivery.Fid);   //获取表头单行数据
                    ...
                    var entityList = delivery.sli_sal_deliverynoticentry_view.ToList();
                    //var index = 0;
                    ...
                    rootObject.Model.FCustomerID.FNumber = delivery.Fcustno;
                    rootObject.Model.FReceiverID.FNumber = delivery.Fcustno;
                    rootObject.Model.FSettleID.FNumber = delivery.Fcustno;
                    rootObject.Model.FPayerID.FNumber = delivery.Fcustno;
                    ...
                    foreach (var entitydata in entityList)
                    {
                        ...
                        newEntry.FMaterialID = new Numberdelivery { FNumber = entitydata.Fnumber };
                        newEntry.FUnitID = new Numberdelivery { FNumber = entitydata.Funitno};   //考虑是否写死，需要更新
                        newEntry.FInventoryQty = 0;
                        newEntry.FRealQty = entitydata.Fqty;
                        newEntry.FDisPriceQty = 0;
                        newEntry.FPrice = 0;
                        newEntry.FTaxPrice = 0;
                        newEntry.FIsFree = false;
                        newEntry.FOwnerTypeID = "BD_OwnerOrg";
                        newEntry.FOwnerID = new Numberdelivery { FNumber = "100" };
                        newEntry.FLot = new Numberdelivery { FNumber = entitydata.Fproductno };   //需要调整为批号
                        newEntry.FEntryTaxRate = 13;
                        newEntry.FAuxUnitQty = entitydata.Fqty;   //需要更新取数逻辑
                        newEntry.FExtAuxUnitId = new Numberdelivery { FNumber = entitydata.Funitno };//考虑是否写死，需要更新
                        ...
                        newEntry.FSrcType = "";
                        newEntry.FSrcBillNo = "";
                        newEntry.FDiscountRate = 0;
                        newEntry.FPriceDiscount = 0;
                        newEntry.FActQty = 0;
                        newEntry.FSalUnitID = new Numberdelivery { FNumber = entitydata.Funitno };//考虑是否写死，需要更新
                        ...
                        newEntry.FBeforeDisPriceQty = 0;
                        newEntry.FSignQty = 0;
                        newEntry.FCheckDelivery = false;
                        newEntry.FAllAmountExceptDisCount = 0;
                        newEntry.FSettleBySon = false;
                        newEntry.FBOMEntryId = 0;
                        newEntry.FMaterialID_Sal = new Numberdelivery { FNumber = entitydata.Fnumber };
                        ...
                        newEntry.FEntity_Link = new List<FEntity_Link>();
                        ...
                        newEntry_Link.FEntity_Link_FSBillId = entitydata.Fid;// 源单内码
                        newEntry_Link.FEntity_Link_FSId = entitydata.Fentryid;//源单分录内码
                        newEntry_Link.FEntity_Link_FBaseUnitQtyOld = entitydata.Fqty;//原始携带量
                        newEntry_Link.FEntity_Link_FBaseUnitQty = entitydata.Fqty;//	 修改携带量
                        newEntry_Link.FEntity_Link_FSALBASEQTYOld = entitydata.Fqty;//	 原始携带量
                        newEntry_Link.FEntity_Link_FSALBASEQTY = entitydata.Fqty;//	 修改携带量
