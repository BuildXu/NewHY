...
{
    //新增材质
    public class sli_bd_metelController : ApiController
    {
        // GET api/<controller>
        public sli_bd_metelController()
        {
            // _context = context;

        }
        /// <summary>
        /// 根据导入销售订单的ID，获取行中的材质信息，判断是否存在，不存在测新增
        /// </summary>
        /// <param name="id"></param>
        /// <returns></returns>
        ...
        public IHttpActionResult DateSync(int id)
        {
            try
            {
                ...
                if (bLogin)
                {
                    ...
                    var numbers = entityList.Select(e => e.FsliMetal).ToList();
                    var prebdoneList = context.T_BAS_PREBDONE
                        .Where(p => numbers.Contains(p.FNUMBER))
                        .Select(p => p.FNUMBER)
                        .ToList();
                    ...
                    {
                        //var PREBDONE = context.T_BAS_PREBDONE.Where(p => p.FNUMBER == entitydata.FsliMetal).ToList();
                        if (prebdoneList.Contains(entitydata.FsliMetal))
                        {
                            continue;
                        }
                        //SaleOrderEntry newEntry = new SaleOrderEntry();
                       
                        rootObject.Model.FName = entitydata.FsliMetal;
                        rootObject.Model.FNumber = entitydata.FsliMetal;
                        ...
                        new object[] { "BAS_PreBaseDataOne", newJson });
                        ...
                        if (!resultdata.Result.ResponseStatus.IsSuccess)
                        {
                            ...
                            {
                                code = 200,
                                msg = resultdata.Result.ResponseStatus.Errors,
                                date = "新增材质失败"
                            };
                            return Ok(dataerr);
                        }
